...
export function resolveRangeByPeriod(period: QueryPeriod): { start: string; end: string; i: IterationOption } {
  switch (period) {
    case '26W':
      return {
        start: moment().add(-26, 'weeks').format('YYYYMMDD'),
        end: moment().format('YYYYMMDD'),
        i: 30
      };
    case '13W':
      return {
        start: moment().add(-13, 'weeks').format('YYYYMMDD'),
        end: moment().format('YYYYMMDD'),
        i: 7
      };
    case '5W':
      return {
        start: moment().add(-5, 'weeks').format('YYYYMMDD'),
        end: moment().format('YYYYMMDD'),
        i: 7
      };
    case '2W':
      return {
        start: moment().add(-2, 'weeks').format('YYYYMMDD'),
        end: moment().format('YYYYMMDD'),
        i: 1
      };
    case '1W':
    default:
      return {
        start: moment().add(-1, 'weeks').format('YYYYMMDD'),
        end: moment().format('YYYYMMDD'),
        i: 1
      };
  }
